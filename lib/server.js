import { getAccessToken } from './local.js';
...
export const updateDict = async (dict) => await fetch(`${baseUrl}/v1/dict/${dict.id}`, fetchInit(true, 'PATCH', dict));

export const updateTask = async (task) => await fetch(`${baseUrl}/v1/task/${task.id}`, fetchInit(true, 'PUT', task));