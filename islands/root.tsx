...
export type Loca = 'waiting'|'about'|'stats'|'dict'|'tasks'|'menu';
...
export default () => {
    ...
    const startStudy = async (ts: Array<IStudy>) => {
        studies.value = ts;
        layers.value = [...layers.value, <Study studies={studies} showTips={showTips} onFinish={handleStudyFinish}/>];
    };
    ...
    const handleMenuClick = async (ev: Event) => {
        preLoca.value = loca.value;
        const title = (ev.target as HTMLMenuElement).title;
        console.log();
        switch (title) {
            case 'issue': return layers.value = [...layers.value, <Issue showTips={showTips} onFinish={goBack}/>];
            case 'setting': return layers.value = [...layers.value, <Setting setting={setting} onFinish={goBack}/>];
            case 'login': return layers.value = [...layers.value, <Signin user={user} showTips={showTips} onFinish={goBack}/>];
            case 'logout': return layers.value = [...layers.value, <Signout onFinish={goBack} handleSignoutClick={handleSignoutClick}/>];
