...
import { totalTask } from "../lib/istat.ts";
import { ISetting } from "../lib/isetting.ts";
import { signals, Dial, startStudy, getUser, getSetting, getStats, showDialog, hideTips, init, close, syncTasks, totalStats } from "../lib/mem.ts";
...
export default () => {
    ...
    return <>
        ...
        {signals.user.value && <div class="fixed inset-x-0 bottom-0 px-4 pb-2 bg-slate-300 dark:bg-slate-700 fill-slate-800 dark:fill-slate-300 flex gap-3 justify-between">
            ...
            <RButton class="stroke-slate-800 dark:stroke-slate-300 grow" onClick={()=>showDialog('menu')}><svg class="w-9 h-9 inline-block" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" stroke-width="4" fill="none"><circle cx="32" cy="20" r="15"/><path d="M3 57a28 28 0 0158 0z"/></svg>我</RButton>
        </div>}
