...
export default ({title, children, onCancel, noback }: {noback?: boolean}&JSX.HTMLAttributes<HTMLDivElement>) => {
    return <div class="fixed inset-0 flex flex-col">
        <div class="shrink-0 px-2 w-[env(titlebar-area-width,100%)] ml-[env(titlebar-area-x,0)] h-[env(titlebar-area-height,38px)] flex justify-between items-center">
        ...
        <div class="grow overflow-y-auto bg-slate-200 dark:bg-slate-800">{children}</div>
    </div>
}